...
const driveFromThree = (cam: VoxelCam, camera: THREE.PerspectiveCamera, cx: number, cz: number) => {
        copyMVP(cam, camera, cx, cz)
        cam.pos[0] = camera.position.x + cx
        cam.pos[1] = camera.position.y
        cam.pos[2] = camera.position.z + cz
        camera.getWorldDirection(_dir)
        cam.eye[0] = cam.pos[0] + _dir.x * 10
        cam.eye[1] = cam.pos[1] + _dir.y * 10
        cam.eye[2] = cam.pos[2] + _dir.z * 10
}

type BaseRenderer = Parameters<THREE.Object3D['onBeforeRender']>[0]

const copyMVP = (cam: VoxelCam, camera: THREE.PerspectiveCamera, cx: number, cz: number) => {
...
const driveFromVoxel = (cam: VoxelCam, camera: THREE.PerspectiveCamera, cx: number, cz: number, aspect: number) => {
        camera.fov = cam.fov()
        camera.near = cam.near()
        camera.aspect = aspect
        camera.position.set(cam.pos[0] - cx, cam.pos[1] + cam.eyeY(), cam.pos[2] - cz)
        _eye.set(cam.eye[0] - cx, cam.eye[1] + cam.eyeY(), cam.eye[2] - cz)
        camera.lookAt(_eye)
...
export class Voxel extends THREE.InstancedMesh {
        ...
        syncCamera(renderer: THREE.Renderer | BaseRenderer, camera: THREE.Camera) {
                const v = this.voxel
                ...
                if (this.controls === 'three') {
                        v.cam.sync(() => driveFromThree(v.cam, persp, cx, cz))
                        v.cam.update(aspect)
                        return
                }
                v.cam.sync((a = aspect) => driveFromVoxel(v.cam, persp, cx, cz, a))
                v.cam.update(aspect)
        }
        onBeforeRender(renderer: THREE.Renderer | BaseRenderer, _scene: THREE.Scene, camera: THREE.Camera) {
                const v = this.voxel
                this.syncCamera(renderer, camera)
