...
import { createCamera, createMesh, createScene, SCOPE, REGION } from 'voxelized-js'
...
const mercator2position = (x = 116415, y = 51623) => {
        x -= SCOPE.x0
        y -= SCOPE.y0
        x += 0.5
        y += 0.5
        x *= REGION
        y *= REGION
        return { X: x, Y: 800, Z: y }
}

// @ts-ignore
const worker = new VoxelWorker()
const cam = createCamera({ ...mercator2position(), yaw: 0, pitch: -Math.PI / 2 + 0.01, mode: -1 })
...
const gl = createGL({
        ...
        render() {
                ...
                cam.update((aspect = gl.size[0] / gl.size[1]))
                node.iMVP.value = [...cam.MVP]
