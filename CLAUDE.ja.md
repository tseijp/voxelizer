...
Region は level (処理完了度)、request (現在の要求)、isError (永続エラー) の内部状態を持つ。
level は 'none' → 'image' → 'full' と進行し、MAX_RETRY (3) 回失敗すると 'none' → 'error' になる。
カメラから離れると dispose により 'none' に戻る。
...
Slot は Region とテクスチャスロットの対応を管理する (デフォルト 16 スロット)。
visible な Region に順次割り当て、SlotUpdate オブジェクトを生成する。
テクスチャのアップロードは利用側 (glre, THREE.js 等) が SlotUpdate を受け取って実行する。
...
mode='scroll' は俯瞰スクロール、mode='creative' は自由飛行、mode='survive' は一人称歩行である。
mode='survive' では Collider による AABB 衝突判定と重力が適用される。
...
| mode       | 'scroll'        | カメラモード文字列      |
| autoScroll | false           | scroll モードの自動移動 |
| move       | 12              | 移動速度 (m/s)          |
| dash       | 3               | ダッシュ倍率            |
| jump       | 12              | ジャンプ初速 (m/s)      |
...
const render = () => {
        ...
        voxel.updates(({ at, atlas, offset }) => {
                // 利用側がテクスチャアップロードを実行 (glre, THREE.js 等)
        })
        if (voxel.updated()) {
...
| -------------- | ------ | --------------------- | ---------------- |
| createVoxel    | Voxel  | Worker, CameraConfig? | 1 canvas に 1 個 |
